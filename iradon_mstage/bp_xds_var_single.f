...
C
      ...
      integer	image_pr
	  integer ndims
      integer dims_image(2)
C-----------------------------------------------------------------------
      integer nlhs, nrhs
      integer mxGetM, mxGetN, mxIsNumeric
      integer m, n, size, image_size
C	real*8 nplr, naz, nspec, nbins, imtype, plri, azii, speci, bc
	real*4 nplr, naz, nspec, nbins, imtype, plri, azii, speci, bc
...
C	call mxCopyPtrToReal8(nplr_pr,nplr,1)
C	call mxCopyPtrToReal8(naz_pr,naz,1)
C	call mxCopyPtrToReal8(nspec_pr,nspec,1)
C	call mxCopyPtrToReal8(nbins_pr,nbins,1)
C	call mxCopyPtrToReal8(imtype_pr,imtype,1)
	call mxCopyPtrToReal4(nplr_pr,nplr,1)
	call mxCopyPtrToReal4(naz_pr,naz,1)
	call mxCopyPtrToReal4(nspec_pr,nspec,1)
	call mxCopyPtrToReal4(nbins_pr,nbins,1)
	call mxCopyPtrToReal4(imtype_pr,imtype,1)
...
C	call mxCopyPtrToReal8(plri_pr,plri,1)
C	call mxCopyPtrToReal8(azii_pr,azii,1)
C	call mxCopyPtrToReal8(speci_pr,speci,1)
C	call mxCopyPtrToReal8(bc_pr,bc,1)
	call mxCopyPtrToReal4(plri_pr,plri,1)
	call mxCopyPtrToReal4(azii_pr,azii,1)
	call mxCopyPtrToReal4(speci_pr,speci,1)
	call mxCopyPtrToReal4(bc_pr,bc,1)
...
C      plhs(1) = mxCreateDoubleMatrix(image_size,1,0)
        dims_image(1) = image_size
        dims_image(2) = 1
        nclassid=mxClassIDFromClassName('single')
        plhs(1) = mxCreateNumericArray(2,dims_image,nclassid,0)
...
C	real*8 nplr, naz, nspec, pri, ai, si, nbins, bc, imt
	real*4 nplr, naz, nspec, pri, ai, si, nbins, bc, imt
	integer mtnang,mpnang,manang,kmd,imtype,nmd,ndimu,kdimu
C	real*8 theta0,phi0,alpha0,center,fkhalf,fnhalf
	real*4 theta0,phi0,alpha0,center,fkhalf,fnhalf,dtheta,dphi,dalpha,s,b,x,y,xi
	real*4, allocatable :: btemp1(:,:,:,:)
	real*4, allocatable :: btemp2(:,:,:,:)
	real*4 sino(*), image(*)
	character*20 string
