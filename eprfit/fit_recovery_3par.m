...
function [fit_amp, fit_t1, fit_inv, fit_err_mask, fit_error, fit_recovery] = fit_recovery_3par(fit_y, TT)
...
[est_TT, err] = fminsearch(fit_fun, [max(p),1/(Trange/5),max(p)], opt);
...
try
  ...
  fit_amp = x(:, 1)'; fit_t1 = 1./(x(:, 2)'); fit_inv = x(:, 3)';
