function varargout = HistoAlignPLG(varargin)
% HISTOALIGNPLG MATLAB code for HistoAlignPLG.fig
%      HISTOALIGNPLG, by itself, creates a new HISTOALIGNPLG or raises the existing
%      singleton*.
%
%      H = HISTOALIGNPLG returns the handle to a new HISTOALIGNPLG or the handle to
%      the existing singleton*.
%
%      HISTOALIGNPLG('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in HISTOALIGNPLG.M with the given input arguments.
%
%      HISTOALIGNPLG('Property','Value',...) creates a new HISTOALIGNPLG or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before HistoAlignPLG_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to HistoAlignPLG_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help HistoAlignPLG

% Last Modified by GUIDE v2.5 16-Feb-2015 10:34:50

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @HistoAlignPLG_OpeningFcn, ...
                   'gui_OutputFcn',  @HistoAlignPLG_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --------------------------------------------------------------------
function HistoAlignPLG_OpeningFcn(hObject, eventdata, handles, varargin)
...
% --------------------------------------------------------------------
function varargout = HistoAlignPLG_OutputFcn(hObject, eventdata, handles) 
...
for ii=1:length(handles.find_list2D)
  if mod(ii, 2) == 0, prefix = 'T'; else prefix = 'H'; end
  str{end+1} = ['Slice ', num2str(round(ii/2)), prefix, ': image ''', handles.find_list2D{ii}.FullName, ''''];
end
...
% convert Z-transformation into slab thinckness and blade width
blade = [];
slab = [];
for ii=3:2:length(z)
  blade(end+1) = z(ii)-z(ii-1);
end
for ii=2:2:length(z)
  slab(end+1) = z(ii) - z(ii-1);
end
blade = mean(blade);

set(handles.eBlade, 'String', num2str(blade));
set(handles.eSliceThick, 'String', sprintf('%g,', slab)); 
...
slab_thickness = str2num(get(handles.eSliceThick, 'String')); %#ok<ST2NM>
slab_thickness(20) = 0;
cut_thickness = str2double(get(handles.eBlade, 'String'));
zpos = 0;
for ii=1:length(slab_thickness)
  zpos(end+1) = zpos(end) + slab_thickness(ii);
  zpos(end+1) = zpos(end) + cut_thickness;
end
